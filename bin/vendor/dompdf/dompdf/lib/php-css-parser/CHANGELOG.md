...
* Unrecoverable parser errors throw an exception of type `schweikert\CSS\Parsing\SourceException` instead of `\Exception`.
...
* The `schweikert\CSS\Value\String` class has been renamed to `schweikert\CSS\Value\CSSString`.
...
* Format output using schweikert\CSS\OutputFormat
* *No backwards-incompatible changes*
...
* `schweikert\CSS\Value\Color`’s `__toString` method overrides `CSSList`’s to maybe return something other than `type(value, …)` (see above).
...
* Introduce a specialized exception (schweikert\CSS\Parsing\OuputException) for exceptions during output rendering
...
* Generic interface `schweikert\CSS\Property\AtRule`, implemented by all @-rule classes
* *No deprecations*

#### Backwards-incompatible changes

* `schweikert\CSS\RuleSet\AtRule` renamed to `schweikert\CSS\RuleSet\AtRuleSet`
* `schweikert\CSS\CSSList\MediaQuery` renamed to `schweikert\CSS\RuleSet\CSSList\AtRuleBlockList` with differing semantics and API (which also works for other block-list-based @-rules like `@supports`).
...
* All properties (like whether or not to use `mb_`-functions, which default charset to use and – new – whether or not to be forgiving when parsing) are now encapsulated in an instance of `schweikert\CSS\Settings` which can be passed as the second argument to `schweikert\CSS\Parser->__construct()`.
* Specifying a charset as the second argument to `schweikert\CSS\Parser->__construct()` is no longer supported. Use `schweikert\CSS\Settings::create()->withDefaultCharset('some-charset')` instead.
* Setting `schweikert\CSS\Parser->bUseMbFunctions` has no effect. Use `schweikert\CSS\Settings::create()->withMultibyteSupport(true/false)` instead.
* `schweikert\CSS\Parser->parse()` may throw a `schweikert\CSS\Parsing\UnexpectedTokenException` when in strict parsing mode.
...
* `schweikert\CSS\RuleSet->getRules()` returns an index-based array instead of an associative array. Use `schweikert\CSS\RuleSet->getRulesAssoc()` (which eliminates duplicate rules and lets the later rule of the same name win).
* `schweikert\CSS\RuleSet->removeRule()` works as it did before except when passed an instance of `schweikert\CSS\Rule\Rule`, in which case it would only remove the exact rule given instead of all the rules of the same type. To get the old behaviour, use `schweikert\CSS\RuleSet->removeRule($oRule->getRule()`;
