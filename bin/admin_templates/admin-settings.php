...
if ( SGVC_testdb() == true ) {
	...
	if ( $datData !== '' ) {
		...
		if ( $tmezon == '' ) {
		   ...
		   $valueNew = array(
			   'Gift_Voucher_Shop'  => $SGVC_valuIS[0],
...
function SGVC_openSettings_funct( $tableIs, $SGVC_valuIS, $fulllogo, $IdLogo, $datdata, $tmeScales ) {
	...
	if ( count( $numaARR ) == 1 ) {
		$numaARR[0] = 'months';
		$numaARR[1] = '6';  }
	$choiceScale = "<select size='1' id='timecapsule' onchange=\"javascript:SGVC_setscales('$numaARR[0]')\" >";
	$cusChoice   = '';
...
echo SGVC_enqueue_func() . SGVC_openSettings_funct( $tableIs, $SGVC_valuIS, $fulllogo, $IdLogo, $datdata, $tmeScales );
